...
#pragma mark - sdk方式分享
- (void)shareNewsWithContent:(SMContent *)content
           completionBlock:(ShareWeiboBlock)aCompletionQQBlock
               failedBlock:(ShareWeiboBlock)aFailedQQBlock
{
    _content = content;
    
    WBMessageObject *message = [self messageWithDescription:_content.desc
                                                  imageData:_content.image.compressedData];
    
    WBAuthorizeRequest *authRequest = [WBAuthorizeRequest request];
    authRequest.redirectURI = _redirectUri;
...
- (WBMessageObject *)messageWithDescription:(NSString *)description
                                  imageData:(NSData *)imageData
{
    WBMessageObject *message = [WBMessageObject message];
    
    message.text = [NSString stringWithFormat:@"%@", description];
...
- (void)didReceiveWeiboResponse:(WBBaseResponse *)response
{
    if ([response isKindOfClass:WBSendMessageToWeiboResponse.class])
    {
        if (response.statusCode == WeiboSDKResponseStatusCodeSuccess) {
            ...
            _accessToken = [sendMessageToWeiboResponse.authResponse accessToken];
...
- (void)shareOAuthNewsWithContent:(SMContent *)content
